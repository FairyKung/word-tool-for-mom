...
/* ── Buttons ── */
.stButton > button {
    border-radius: 12px !important; font-family: 'Sarabun', sans-serif !important;
    font-size: 1.05rem !important; font-weight: 600 !important;
    padding: 0.55rem 1.2rem !important; border: none !important; transition: all 0.2s ease !important;
}
.stButton > button[kind="primary"] {
    background: linear-gradient(135deg, #6c63ff, #4e46d4) !important;
    color: white !important; width: 100% !important; padding: 0.75rem !important;
    font-size: 1.15rem !important; box-shadow: 0 4px 14px rgba(108,99,255,0.35) !important;
}
.stButton > button[kind="primary"]:hover { transform: translateY(-2px) !important; }
.stButton > button:not([kind="primary"]) {
    background: rgba(255,255,255,0.2) !important;
    color: white !important; border: 1.5px solid rgba(255,255,255,0.5) !important;
}

/* ── File uploader ── */
[data-testid="stFileUploader"] {
    background: white !important; border: 2.5px dashed #6c63ff !important;
...
.paper-page {
    background: white; border-radius: 8px; padding: 48px 52px;
    box-shadow: 0 4px 20px rgba(0,0,0,0.12); min-height: 600px;
    max-width: 820px; margin: 0 auto;
    font-family: 'Sarabun', sans-serif !important; font-size: 16px; line-height: 1.9; color: #111;
}
...
[data-testid="stDownloadButton"] > button {
    ...
    padding: 0.75rem 1.5rem !important; border-radius: 12px !important;
...
# ══════════════════════════════════════════════════════════════
#  REPLACE ENGINE  (แก้ครบทุกปัญหา)
#  ✅ Clone rPr XML เต็ม → ฟอนต์/ขนาดไม่เพี้ยน
#  ✅ รวม run ก่อน regex → เปลี่ยนคำที่ถูกหั่น run ได้
#  ✅ Skip duplicate merged cells → layout ไม่เลื่อน
# ══════════════════════════════════════════════════════════════
...
def _replace_in_paragraph(paragraph, old_text, new_text):
    ...
    for m in matches:
        ...
        if new_text:
            if segments and segments[-1][1] == first_r:
                segments[-1] = (segments[-1][0] + new_text, first_r)
...
def _replace_in_table(table, old, new):
    """วน loop โดย skip merged cells ที่ซ้ำ"""
    count = 0
    for row in table.rows:
        seen_tc = set()
        for cell in row.cells:
            tc_id = id(cell._tc)
            if tc_id in seen_tc: continue   # ← skip duplicate merged cell
            seen_tc.add(tc_id)
